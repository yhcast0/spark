...
object IteratorSerializerUtils {
  ...
  def serialize(iterator: Iterator[ByteBuffer], count: Int): ChunkedByteBuffer = {
    val array = iterator.toArray
    val chunkSize = ChunkedByteBuffer.estimateBufferChunkSize()
    val cbbos = new ChunkedByteBufferOutputStream(chunkSize, ByteBuffer.allocate)
    cbbos.write(intToByteArrayBigEndian(count))
    for (buffer <- array) {
      cbbos.write(buffer.array())
    }
    cbbos.close()
