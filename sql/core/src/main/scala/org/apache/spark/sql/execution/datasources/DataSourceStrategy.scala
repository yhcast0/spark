...
import org.apache.spark.sql.catalyst.plans.logical.{AppendData, InsertIntoDir, InsertIntoStatement, LogicalPlan, Project, RebalancePartitions, RepartitionByExpression, RepartitionOperation, Sort}
  ...
  private def applyRepartition(
    ...
    (bucketSpec, partitionColumns) match {
      case (None, partExps) if partExps.nonEmpty =>
        query match {
          case RepartitionByExpression(partitionExpressions, _, _, _)
      ...
      case (Some(bucket), partExps) =>
        ...
        if (bucket.sortColumnNames.nonEmpty) {
          ...
          query match {
            case Sort(order, false,
            RepartitionByExpression(partExpr, _, Some(bucket.numBuckets), _))
          ...
          query match {
            case RepartitionByExpression(partExpr, _, Some(bucket.numBuckets), _)
