 ...
 */
class ReadOnlySQLConf(newSettings: JMap[String, String], newReader: ConfigReader) extends SQLConf {

  @transient override val settings: JMap[String, String] = newSettings

  @transient override protected val reader: ConfigReader = newReader

  def this(context: TaskContext) {
    this(context.getLocalProperties.asInstanceOf[JMap[String, String]],
      new ConfigReader(new TaskContextConfigProvider(context)))
  }

  def this(context: SparkContext) {
    this(context.getConf.getAll.toMap.asJava,
      new ConfigReader(new SparkContextConfigProvider(context)))
  }
...
class SparkContextConfigProvider(context: SparkContext) extends ConfigProvider {
  override def get(key: String): Option[String] = context.getConf.getOption(key)
}
